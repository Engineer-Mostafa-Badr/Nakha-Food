...
class FirebaseNotificationsHandlerPlus extends StatefulWidget {
  /// {@template fcmToken}
  ...
  ///   * [onTap] parameter.
  /// {@endtemplate}
  final void Function(GlobalKey<NavigatorState> navigatorKey, Map payload)?
  ...
  const FirebaseNotificationsHandlerPlus({
    super.key,
    this.vapidKey,
  ...
  @override
  State<FirebaseNotificationsHandlerPlus> createState() =>
      _FirebaseNotificationsHandlerPlusState();
}

class _FirebaseNotificationsHandlerPlusState
    extends State<FirebaseNotificationsHandlerPlus> {
  @override
  void initState() {
    () async {
      ...
      PushNotificationService.onTokenRefresh.listen((token) {
        widget.onFCMTokenUpdate?.call(navKey.currentContext ?? context, token);
      });
    }();
